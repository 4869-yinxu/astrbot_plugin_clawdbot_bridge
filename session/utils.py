...
def extract_user_id(event: AstrMessageEvent, group_id: str = "") -> str:
    ...
    if hasattr(event, 'message_obj') and hasattr(event.message_obj, 'sender'):
        try:
            ...
            if sender_user_id and sender_user_id != group_id:
                logger.debug(f"[session] 从 message_obj.sender.user_id 获取: {sender_user_id}")
    ...
    if event.session_id:
        parts = str(event.session_id).split('_')
        if parts:
            logger.warning(f"[session] 使用 session_id 最后部分作为用户 ID")
            return parts[-1]
...
def _extract_user_id_from_raw(raw_msg: Any, group_id: str) -> Optional[str]:
